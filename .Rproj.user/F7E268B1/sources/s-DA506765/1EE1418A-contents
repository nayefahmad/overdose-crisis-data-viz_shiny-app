...
shinyServer(function(input, output, session) {
  ...
  foundational_map <- shiny::reactive({
    ...
    .leaflet <- leaflet() %>%
      ...
      addPolygons(data = sub_world,
                  ...
                  label = sub_world$NAME,
                  group = "click_list")
    
    # Adds current selection
    to_add <- isolate(click_list$ids)
    if (length(to_add) > 0) {
      for (id in to_add) {
  ...
  observe({
    ...
    if (identical(old_selection, new_selection)) {
      return()
    }
    
    # Create list for removal and addition 
    to_remove <- setdiff(old_selection, new_selection)
    to_add <- setdiff(new_selection, old_selection)
